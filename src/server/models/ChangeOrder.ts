import { query, db } from '../../database/db';
import { ChangeOrder as ChangeOrderType } from '../../types';
...
interface UpdateChangeOrderParams {
  title?: string;
  description?: string;
  price?: number;
  status?: string;
...
class ChangeOrder {
  // Create change order
  static create({ project_id, title, description, price, request_ids }: CreateChangeOrderParams) {
    // Start transaction
    ...
    try {
      ...
      const changeOrderId = Number(result.lastInsertRowid);
  ...
  static findByProjectId(projectId: number, status: string | null = null) {
    if (status) {
      return query.all(
  ...
  // Update change order
  static update(id: number, { title, description, price, status }: UpdateChangeOrderParams) {
  ...
  static approve(id: number, approvedBy: string = 'freelancer', clientNotes: string | null = null) {
    const updates = ['status = ?', 'approved_at = ?', 'approved_by = ?'];
    const values: any[] = ['approved', Math.floor(Date.now() / 1000), approvedBy];
  ...
  // Reject change order
  static reject(id: number, clientNotes: string | null = null) {
    const updates = ['status = ?', 'rejected_at = ?'];
    const values: any[] = ['rejected', Math.floor(Date.now() / 1000)];
  ...
  static addClientFeedback(id: number, clientNotes: string) {
    query.run(
  ...
  static delete(id: number) {
    ...
    if (changeOrder && changeOrder.requests) {
      changeOrder.requests.forEach((request: any) => {
        query.run(
  ...
  static generateFromRequests(projectId: number, requestIds: number[], { title, price }: { title: string; price: number }) {
    const Request = require('./Request').default;

    // Verify all requests are out-of-scope
    const requests = requestIds.map(id => Request.findById(id)).filter(Boolean);
    
    const outOfScope = requests.filter((r: any) => r.status === 'out-of-scope');
    ...
    const description = outOfScope
      .map((r: any, i: number) => `${i + 1}. ${r.description}`)
      .join('\n');
