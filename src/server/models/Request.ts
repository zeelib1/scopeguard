...
class Request {
  // Create request
  static create({ project_id, description, source, status, priority, scope_item_id, notes }: CreateRequestParams): RequestType | undefined {
  ...
  // Find by ID
  static findById(id: number): RequestType | undefined {
    return query.get<RequestType>(`SELECT * FROM requests WHERE id = ?`, [id]);
  }
  ...
  // Update request
  static update(id: number, { description, source, status, priority, scope_item_id, notes }: UpdateRequestParams): RequestType | undefined {
  ...
  static categorize(id: number, status: string, scopeItemId: number | null = null): RequestType | undefined {
    if (!['in-scope', 'out-of-scope'].includes(status)) {
      throw new Error('Status must be in-scope or out-of-scope');
    }

    const ScopeItem = require('./ScopeItem').default;
