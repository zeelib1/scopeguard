...
const verifyProjectAccess = (req, res, next) => {
...
router.post('/', verifyAdminPermission, (req, res) => {
  try {
...
router.put('/:memberId', verifyAdminPermission, (req, res) => {
  try {
    const { role } = req.body;
...
router.delete('/:memberId', verifyAdminPermission, (req, res) => {
  try {
    const member = ProjectMember.findById(req.params.memberId);
