...
router.post('/generate', (req: AuthRequest, res: Response): void => {
  try {
    ...
    if (!request_ids || !Array.isArray(request_ids) || request_ids.length === 0) {
      res.status(400).json({ error: 'At least one request ID required' });
    }
...
router.post('/:id/approve', (req: AuthRequest, res: Response): void => {
  try {
    const changeOrder = ChangeOrder.findById(req.params.id);
...
router.post('/:id/reject', (req: AuthRequest, res: Response): void => {
  try {
    const changeOrder = ChangeOrder.findById(req.params.id);
