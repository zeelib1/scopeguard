...
router.post('/', upload.array('attachments', 5), (req: AuthRequest, res: Response): void => {
  try {
...
router.post('/:id/categorize', (req: AuthRequest, res: Response): void => {
  try {
    const request = Request.findById(req.params.id);
    ...
    if (!status || !['in-scope', 'out-of-scope'].includes(status)) {
      res.status(400).json({ error: 'Status must be in-scope or out-of-scope' });
    }
...
router.get('/:id/attachments', (req: AuthRequest, res: Response): void => {
  try {
    const request = Request.findById(req.params.id);
...
router.post('/:id/attachments', upload.array('attachments', 5), (req: AuthRequest, res: Response): void => {
  try {
    const request = Request.findById(req.params.id);
...
router.get('/:id/attachments/:attachmentId/download', (req: AuthRequest, res: Response): void => {
  try {
...
router.delete('/:id/attachments/:attachmentId', (req: AuthRequest, res: Response): void => {
  try {
    const attachment = Attachment.findById(req.params.attachmentId);
...
router.get('/:id/time', (req: AuthRequest, res: Response): void => {
  try {
    const request = Request.findById(req.params.id);
...
router.post('/:id/time/start', (req: AuthRequest, res: Response): void => {
  try {
    const request = Request.findById(req.params.id);
...
router.post('/:id/time/stop', (req: AuthRequest, res: Response): void => {
  try {
    const activeTimer = TimeEntry.getActiveTimer(req.user.id);
    ...
    if (activeTimer.request_id !== parseInt(req.params.id)) {
      res.status(400).json({ error: 'Active timer is for a different request' });
    }
...
router.put('/:id/time/:timeId', (req: AuthRequest, res: Response): void => {
  try {
...
router.delete('/:id/time/:timeId', (req: AuthRequest, res: Response): void => {
  try {
    const timeEntry = TimeEntry.findById(req.params.timeId);
